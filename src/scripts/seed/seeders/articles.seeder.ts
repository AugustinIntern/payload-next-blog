...
export async function seedArticles(payload: Payload) {
  ...
  for (let i = 0; i < ARTICLES_COUNT; i++) {
    try {
      ...
      if (!image) {
        console.warn('stopped seeding article author because image could not be created')
        return
      }
      ...
      console.error('Error occurred while seeding article:', error)
    }
  }
  console.log(`${successCount}/${ARTICLES_COUNT} articles seeded successfully`)
}
